...
//Transposing the matrix was cool, but not strictly necessary and 
//I had a hard time with this one at first. Misunderstood the algorithm - I was counting the most common bit in each position in the original dataset. I should have been counting the most common bit **remaining** in the filtered data set.
...
for (let i = 0; i < numOfBits; i++) {
    if (tempOxyData.length > 1) {
        ...
        const mostCommonBit = (countBit1 >= (tempOxyData.length - countBit1)) ? 1 : 0; //are 1s or 0s more common?

        tempOxyData = tempOxyData.filter(bin => {
            return bin[i] == mostCommonBit; //keep only lines that have the most common bit at this bit position
        });
    }

    if (tempCO2Data.length > 1) {
        const countBit1 = tempCO2Data.filter(bin => bin[i] == 1).length; //count the number of 1s
        const leastCommonBit = (countBit1 >= (tempCO2Data.length - countBit1)) ? 0 : 1; //are 1s or 0s more common?

        tempCO2Data = tempCO2Data.filter(bin => {
            return bin[i] == leastCommonBit; //keep only lines that have the least common bit at this bit position
        });
    }
}
